...
class _LessonPageState extends ConsumerState<LessonPage> {
    ...
    child: Column(children: [
      Row(children: [
        ...
        Expanded(child: Text(widget.lesson.title, textAlign: TextAlign.center,
          style: const TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 15, fontWeight: FontWeight.w700, color: Colors.white))),
        Padding(padding: const EdgeInsets.only(left: 8),
          child: Text('${qi+1}/$totalQ', style: const TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 13, color: Colors.white))),
      ]),
    ...
    children: List.generate(totalQ, (i) {
      ...
      return Container(width: 24, height: 24,
        ...
        child: Center(child: Text(r == null ? '${i+1}' : (r ? '✓' : '✗'),
          style: TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 11, fontWeight: FontWeight.w700, color: fg))));
    }),
  );
        ...
        children: choices.map((c) {
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(duration: 200.ms,
              ...
              child: Center(child: Text(c, style: TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 20, fontWeight: FontWeight.w700, color: fg)))));
        }).toList()),
      const SizedBox(height: 16),
  ...
  Widget _buildDigitalMC(TimeQuestion q) {
    ...
    return Column(children: [
      ...
      Text('الوقت بالساعة الرقمية: ${q.time24Str}',
        style: const TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 13, color: WaqtiColors.textLight)),
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(duration: 200.ms,
              ...
              child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text(c.label, style: const TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 10, color: WaqtiColors.textLight), textAlign: TextAlign.center),
              ])));
        }).toList()),
      const SizedBox(height: 16),
      if (answered) _nextBtn(),
  ...
  Widget _buildSetHands(TimeQuestion q) {
    ...
    return Column(children: [
      Text.rich(TextSpan(children: [
        const TextSpan(text: 'حرّك العقارب إلى: ', style: TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 14, color: WaqtiColors.textMid)),
        TextSpan(text: q.timeStr, style: TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 16, fontWeight: FontWeight.w800, color: color)),
      ]), textAlign: TextAlign.center),
      const SizedBox(height: 4),
      Text(q.arabicTime, style: const TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 12, color: WaqtiColors.textLight)),
      const SizedBox(height: 16),
      InteractiveClock(
      ...
      Text('${setH.toString().padLeft(2,'0')}:${setM.toString().padLeft(2,'0')}',
        style: TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 32, fontWeight: FontWeight.w800,
      ...
      if (answered && !correctNow)
        Padding(padding: const EdgeInsets.only(top: 6),
          child: Text('الإجابة الصحيحة: ${q.timeStr} — ${q.arabicTime}',
            textAlign: TextAlign.center,
            style: const TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFFC62828)))),
      const SizedBox(height: 6),
      const Text('☝️ المس الساعة واسحب العقرب الذي تريد تحريكه',
        style: TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 11, color: WaqtiColors.textLight)),
      const SizedBox(height: 16),
      if (!answered)
        ElevatedButton(
          ...
          child: const Text('✓ تحقّق', style: TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 16, fontWeight: FontWeight.w700)),
        )
      else _nextBtn(),
    ]);
  }
  ...
  Widget _buildCalcQ() {
    ...
    return Column(children: [
      Row(mainAxisAlignment: MainAxisAlignment.center, children: [
        Column(children: [
          const Text('البداية', style: TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 11, color: WaqtiColors.textLight)),
          const SizedBox(height: 4),
        ...
        Column(children: [
          const Text('النهاية', style: TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 11, color: WaqtiColors.textLight)),
          const SizedBox(height: 4),
      ...
      Container(padding: const EdgeInsets.symmetric(horizontal: 18, vertical: 10),
        ...
        child: const Text('🧮 كم من الوقت مرّ؟',
          style: TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 15, fontWeight: FontWeight.w700, color: WaqtiColors.textDark))),
      const SizedBox(height: 18),
      GridView.count(crossAxisCount: 2, shrinkWrap: true,
        physics: const NeverScrollableScrollPhysics(),
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(duration: 200.ms,
              decoration: BoxDecoration(
                ...
                boxShadow: [BoxShadow(color: Colors.black.withOpacity(.05), blurRadius: 6, offset: const Offset(0,2))]),
              child: Center(child: Text(opt, style: const TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 18, fontWeight: FontWeight.w700, color: WaqtiColors.textDark)))));
        }).toList()),
      const SizedBox(height: 16),
      if (answered) _nextBtn(),
    ]);
  ...
  Widget _nextBtn() {
    ...
    return ElevatedButton(
      ...
      child: Text(isLast ? 'أكمل الدرس 🎉' : 'السؤال التالي ←',
        style: const TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 16, fontWeight: FontWeight.w700)),
  ...
  Widget _buildFinish() {
    ...
    return SingleChildScrollView(child: Padding(padding: const EdgeInsets.all(28), child: Column(children: [
      ...
      const SizedBox(height: 20),
      const Text('انتهى الدرس!', style: TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 26, fontWeight: FontWeight.w800, color: WaqtiColors.textDark)),
      const SizedBox(height: 12),
      ...
      const SizedBox(height: 10),
      Text('$correct من $totalQ إجابات صحيحة', style: const TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 15, color: WaqtiColors.textMid)),
        ...
        Padding(padding: const EdgeInsets.only(bottom: 10), child: SizedBox(width: double.infinity,
          child: ElevatedButton(
            ...
            child: Text('التالي: ${next.lesson!.title} ${next.unit!.emoji} ←',
              style: const TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 15, fontWeight: FontWeight.w700))))),
      SizedBox(width: double.infinity, child: ElevatedButton(
        ...
        child: const Text('العودة للمسار 🏠', style: TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 15, fontWeight: FontWeight.w700)))),
      const SizedBox(height: 8),
      SizedBox(width: double.infinity, child: OutlinedButton(
        ...
        child: const Text('🔄 أعد الدرس', style: TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 14, fontWeight: FontWeight.w600, color: WaqtiColors.textMid)))),
  ...
  Widget _pill(String t) => Container(
    ...
    child: Text(t, style: const TextStyle(fontFamily: GoogleFonts.cairo().fontFamily, fontSize: 12, fontWeight: FontWeight.w600, color: WaqtiColors.primary)));
