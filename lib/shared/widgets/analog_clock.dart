...
class AnalogClock extends StatelessWidget {
  ...
  Widget build(BuildContext context) => SizedBox(
    ...
    child: CustomPaint(
      painter: _ClockPainter(hour: hour, minute: minute, color: color),
    ),
  );
}

// ══════════════════════════════════════════════════════════════
// Interactive clock — FIXED drag (closest-hand detection)
// ══════════════════════════════════════════════════════════════
...
class _InteractiveClockState extends State<InteractiveClock> {
  late int    _h, _m;
  String?     _dragging;   // 'hour' | 'minute' | null
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 60; i++) {
      ...
      canvas.drawLine(
        ...
        Paint()
          ..color = maj ? color : color.withOpacity(.35)
          ..strokeWidth = maj ? r * .038 : r * .018
    ...
    for (int i = 1; i <= 12; i++) {
      ...
      tp.paint(
        canvas,
        Offset(cx + nr * math.cos(a), cx + nr * math.sin(a)) -
